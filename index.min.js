...
function fromJson(collection, collectionPath, next) {
    ...
    return 1 > last ? next() : void docs.forEach(function(docName) {
        ...
        collection.save(doc, function(err) {
            return null !== err ? last === ++index ? next(err) : error(err) : last === ++index ? next() : null;
        });
    });
}
...
function allCollections(db, name, metadata, parser, next) {
    ...
    last--), void collections.forEach(function(collectionName) {
        ...
        db.createCollection(collectionName, function(err, collection) {
            return null !== err ? last === ++index ? next(err) : error(err) : (logger("select collection " + collectionName), 
...
function wrapper(my) {
    function callback() {
        logger("restore stop"), null !== my.tar && rmDir(my.dir), null !== my.callback && (logger("callback run"), 
        my.callback());
    }
    var parser;
    if ("function" == typeof my.parser) parser = my.parser; else switch (my.parser) {
      case "bson":
        BSON = require("bson").BSONPure.BSON, parser = fromBson;
        break;
    ...
    var metadata = "";
    meta = my.metadata === !0 ? readMetadata : function(a, b, c) {
        return c();
    };
    var go = function(root) {
        my.metadata === !0 && (metadata = root + ".metadata/"), require("mongodb").MongoClient.connect(my.uri, my.options, function(err, db) {
            return logger("db open"), null !== err ? error(err) : void discriminator(db, root, metadata, parser, function(err) {
                null !== err && error(err), logger("db close"), db.close(), callback();
            });
        });
    };
    null === my.tar ? go(my.root) : (logger("open tar file at " + my.root + my.tar), 
    makeDir(my.dir, function() {
        ...
        my.stream ? my.stream.pipe(extractor) : fs.createReadStream(my.root + my.tar).on("error", error).pipe(extractor);
    }));
}
