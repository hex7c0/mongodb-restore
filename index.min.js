...
function readMetadata(collection, metadata, next) {
    ...
    return 0 === last ? next(null) : void doc.forEach(function(index) {
        collection.createIndex(index.key, index, function(err) {
            return err ? last === ++counter ? next(err) : error(err) : void (last === ++counter ? next(null) : null);
        });
    });
}
...
function rmDir(pathname, next) {
    fs.readdirSync(pathname).forEach(function(first) {
        ...
        if (fs.statSync(database).isDirectory() !== !1) {
            ...
            fs.existsSync(metadataPath) === !0 && (metadata = metadataPath + path.sep, delete collections[collections.indexOf(".metadata")]), 
...
function fromJson(collection, collectionPath, next) {
    ...
    return 0 === last ? next(null) : void docs.forEach(function(docName) {
        var doc, data;
        try {
...
function allCollections(db, name, metadata, parser, next) {
    ...
    last--), void collections.forEach(function(collectionName) {
        ...
        db.createCollection(collectionName, function(err, collection) {
            return err ? last === ++counter ? next(err) : error(err) : (logger("select collection " + collectionName), 
            void meta(collection, metadata, function(err) {
...
function someCollections(db, collections, next) {
    ...
    return 0 === last ? next(null) : void collections.forEach(function(collection) {
        db.collection(collection, function(err, collection) {
            return logger("select collection " + collection.collectionName), err ? last === ++counter ? next(err) : error(err) : void collection.drop(function(err) {
                err && error(err), last === ++counter ? next(null) : null;
            });
        });
    });
}
...
function wrapper(my) {
    ...
    function go(root) {
        my.metadata === !0 && (metadata = path.join(root, ".metadata", path.sep)), require("mongodb").MongoClient.connect(my.uri, my.options, function(err, db) {
            function next(err) {
    ...
    return meta = my.metadata === !0 ? readMetadata : function(a, b, c) {
        ...
        var extractor = require("tar").Extract({
            ...
            for (var dirs = fs.readdirSync(my.dir), i = 0, ii = dirs.length; i < ii; ++i) {
                var t = my.dir + dirs[i];
                if (fs.statSync(t).isFile() === !1) return go(t + path.sep);
            }
        });
