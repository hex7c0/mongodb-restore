...
function readMetadata(collection, metadata, next) {
    var indexes, t = metadata + collection.collectionName;
    ...
    return 0 === last ? next(null) : void indexes.forEach(function(index) {
...
function fromJson(collection, collectionPath, next) {
    ...
    return 0 === last ? next(null) : void docs.forEach(function(docName) {
        ...
        if (fs.statSync(docPath).isFile() === !1) return last === ++index ? next(null) : null;
        try {
        ...
        db.createCollection(collectionName, function(err, collection) {
            ...
            void meta(collection, metadata, function(err) {
                err && error(err), parser(collection, collectionPath + path.sep, function(err) {
                    return err ? last === ++index ? next(err) : error(err) : void (last === ++index ? next(null) : null);
                });
            }));
        });
    }));
    ...
    return 0 === last ? next(null) : void collections.forEach(function(collection) {
        db.collection(collection, function(err, collection) {
            return logger("select collection " + collection.collectionName), err ? last === ++index ? next(err) : error(err) : void collection.drop(function(err) {
                err && error(err), last === ++index ? next(null) : null;
            });
        });
    });
}
